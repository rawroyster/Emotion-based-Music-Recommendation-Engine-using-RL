...
class Environment():
    ...
    def get_groups(self):
        ...
        for rewards, group in self.embedded_data.groupby(['reward']):
            size = group.shape[0]
            states = np.array(group['state'].to_list())
            actions = np.array(group['action'].to_list())
            groups.append({
                'size': size,
                'rewards': rewards,
                'average state': (
                        np.sum(states / np.linalg.norm(states, 2, axis=1)[:, np.newaxis], axis=0) / size).reshape(
                    (1, -1)),  # s_x^-
                'average action': (
                        np.sum(actions / np.linalg.norm(actions, 2, axis=1)[:, np.newaxis], axis=0) / size).reshape(
                    (1, -1))  # a_x^-
            })
        return groups
    ...
    def simulate_rewards(self, current_state, chosen_actions, reward_type='grouped cosine'):
        ...
        if reward_type == 'normal':
            ...
            probabilities = np.array([g['size'] for g in self.groups]) * [(self.alpha * (
                    np.dot(current_state, g['average state'].T) / np.linalg.norm(current_state, 2)) + (
                                                                                   1 - self.alpha) * (
                                                                                   np.dot(chosen_actions, g[
                                                                                       'average action'].T) / np.linalg.norm(
                                                                               chosen_actions, 2)))
        ...
        def overall_reward(rewards, gamma):
            ...
            for k, reward in enumerate(rewards):
                if isinstance(reward, (int, float)):  # Check if reward is a numeric type
                    total_reward += (gamma ** k) * reward
                else:
                    # Handle non-numeric rewards here, e.g., convert them to numeric values
                    # You can define custom logic for handling non-numeric rewards
                    pass  # Replace this with your logic if needed
            return total_reward
        ...
        if reward_type in ['normal', 'grouped average']:
            ...
            cumulated_reward = np.sum(
                [p * overall_reward(g['rewards'], self.gamma) for p, g in zip(probabilities, self.groups)])
