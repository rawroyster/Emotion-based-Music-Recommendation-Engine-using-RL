    ...
    def __init__(self, sess, state_space_size, action_space_size, batch_size, ra_length, history_length, embedding_size,
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            self.target_network_params = tf.compat.v1.trainable_variables()[
                                         len(self.network_params):]  # TODO: why sublist [len(x):]? Maybe because its equal to network_params + target_network_params
            ...
            self.update_target_network_params = [self.target_network_params[i].assign(
                ...
                tf.multiply(1 - self.tau, self.target_network_params[i])) for i in
            ...
            gradients = tf.gradients(
                tf.reshape(self.action_weights, [self.batch_size, self.action_space_size], name='42'),
                self.network_params, self.action_gradients)
            params_gradients = list(
                map(lambda x: tf.compat.v1.div(x, self.batch_size * self.action_space_size), gradients))
...
class Critic():
    ...
    def __init__(self, sess, state_space_size, action_space_size, history_length, embedding_size, tau, learning_rate,
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            self.loss = tf.reduce_mean(tf.compat.v1.squared_difference(self.expected_reward, self.critic_Q_value))
