...
class DataGenerator():
    ...
    def load_datas(self, datapath, itempath):
        ...
        data = data.groupby(['session_id', 'session_length']).filter(
            lambda x: len(x) == 20)  # Filter out the sessions that are too short
    ...
    def sample_histo(self, user_histo, action_ratio=0.8, max_samp_by_user=10, max_state=10, max_action=5,
        ...
        is random and each sample can have different size for action and state.
        ...
        If given, size of nb_states is the number of sample by user
...
def read_file(data_path):
    ...
    for col in ['state', 'n_state', 'action_reward']:
        data[col] = [np.array([[int(float(k)) for k in ee.split('&')] for ee in e.split('|')]) for e in data[col]]
